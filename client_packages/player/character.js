...
mp.events.add('characterSave', (firstname, secondname, characterage) => {
    if (firstname.length < 3 || firstname.length > 20) return mp.game.graphics.notify('~r~Введите имя от 3 до 20 символов!');
    if (secondname.length < 3 || secondname.length > 20) return mp.game.graphics.notify('~r~Введите фамилию от 3 до 20 символов!');
    if (characterage < 19 || characterage > 99) return mp.game.graphics.notify('~r~Возраст от 19 лет!');
    firstname = firstname[0].toUpperCase() + firstname.substring(1)
    secondname = secondname[0].toUpperCase() + secondname.substring(1)
    var nickname = `${firstname}_${secondname}`
    mp.game.graphics.notify('Сохранение персонажа');
    ...
    hair_or_colors.push(0);
    hair_or_colors.push(hairColor);
    charCreateBrowser.destroy();
    setTimeout(function () {
        mp.events.callRemote("SaveCharacter", nickname, characterage, currentGender, father, mother, similarity, skin, JSON.stringify(features), JSON.stringify(appearance_values), JSON.stringify(hair_or_colors));
    }, 5000);
});
